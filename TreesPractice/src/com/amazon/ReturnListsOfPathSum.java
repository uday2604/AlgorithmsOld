...
public class ReturnListsOfPathSum {
    ...
    public static List<List<Integer>> pathSum(BinaryTreeNode root, int sum) {
        ...
        Queue<Integer> sumTillCurrentNodeList = new LinkedList<>();
        nodes.add(root);
        sumTillCurrentNodeList.add(root.data);
        LinkedList<List<Integer>> l = new LinkedList<List<Integer>>();
        List<Integer> a = new ArrayList<>();
        a.add(root.data);
        l.add(a);
        while (!nodes.isEmpty()) {
            Integer sumTillCurrentNode = sumTillCurrentNodeList.poll();
            BinaryTreeNode t = nodes.poll();
            a = l.poll();
            if (t.llink == null && t.rlink == null && sumTillCurrentNode == sum) {
                result.add(a);
            } else {
                if (t.rlink != null) {
                    nodes.add(t.rlink);
                    sumTillCurrentNodeList.add(t.rlink.data + sumTillCurrentNode);
                ...
                if (t.llink != null) {
                    nodes.add(t.llink);
                    sumTillCurrentNodeList.add(t.llink.data + sumTillCurrentNode);
